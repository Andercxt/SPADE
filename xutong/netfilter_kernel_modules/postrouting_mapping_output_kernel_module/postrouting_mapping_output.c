...
static unsigned int hook_func(void *priv, struct sk_buff *skb, 
	...
	if (iph->protocol == IPPROTO_TCP) {
		tcph = tcp_hdr(skb);
		printk(KERN_INFO "Packet leaving POST_ROUTING");
		...
		for_each_net(net) {
			dev = ip_dev_find(net, iph->saddr);
			if (dev) {
				printk(KERN_INFO "Network namespace for ip %pI4 is %u\n", &iph->saddr, net->ns.inum);
			}
		}
...
static int __init initialize(void) {
       // nfho = (struct nf_hook_ops*)kcalloc(1, sizeof(struct nf_hook_ops), GFP_KERNEL);
        ...
        nfho.hooknum  = NF_INET_POST_ROUTING; //Our function will run at Netfilter's post-routing hook.
